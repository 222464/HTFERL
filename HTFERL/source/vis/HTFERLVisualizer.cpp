#include "HTFERLVisualizer.h"

using namespace vis;

void HTFERLVisualizer::create(unsigned int width) {
	_rt.create(width, width, false);
	_rt.setSmooth(true);
}

void HTFERLVisualizer::update(sf::RenderTexture &target, const sf::Vector2f &position, const sf::Vector2f &scale, sys::ComputeSystem &cs, const htferl::HTFERL &htmrl, std::mt19937 &generator) {
	std::vector<std::shared_ptr<sf::Image>> images;

	htmrl.exportStateData(cs, images, 5634);
